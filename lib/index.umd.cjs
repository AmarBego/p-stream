(function(global, factory) {
  typeof exports === "object" && typeof module !== "undefined" ? factory(exports, require("iso-639-1"), require("wyzie-lib"), require("cheerio"), require("nanoid"), require("unpacker"), require("crypto-js"), require("cookie"), require("set-cookie-parser"), require("abort-controller"), require("form-data")) : typeof define === "function" && define.amd ? define(["exports", "iso-639-1", "wyzie-lib", "cheerio", "nanoid", "unpacker", "crypto-js", "cookie", "set-cookie-parser", "abort-controller", "form-data"], factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, factory(global.index = {}, global["iso-639-1"], global["wyzie-lib"], global.cheerio, global.nanoid, global.unpacker, global["crypto-js"], global.cookie, global["set-cookie-parser"], global["abort-controller"], global["form-data"]));
})(this, function(exports2, ISO6391, wyzieLib, cheerio, nanoid$1, unpacker, crypto, cookie, setCookieParser, AbortController, FormData) {
  "use strict";
  ...
  async function addWyzieCaptions(captions, tmdbId, imdbId, season, episode) {
    try {
      ...
      const wyzieSubtitles = await wyzieLib.searchSubtitles(searchParams);
  ...
  async function addOpenSubtitlesCaptions(captions, ops, media) {
    ...
    try {
      ...
      try {
        ...
        const wyzieCaptions = await Promise.race([wyziePromise, timeout(2e3, "Wyzie")]);
      ...
      try {
        ...
        const Res = await Promise.race([openSubsPromise, timeout(5e3, "OpenSubtitles")]);
        if (!Res) return captions;
  ...
  const baseUrl$c = "https://soaper.cc";
  const universalScraper$4 = async (ctx) => {
    ...
    if (ctx.media.type === "show") {
      ...
      const showPage = await ctx.proxiedFetcher(showLink, { baseUrl: baseUrl$c });
    ...
    if (!showLink) throw new NotFoundError("Content not found");
    const contentPage = await ctx.proxiedFetcher(showLink, { baseUrl: baseUrl$c });
    ...
    return {
      ...
      stream: [
        {
          id: "primary",
          playlist: createM3U8ProxyUrl(`${baseUrl$c}/${streamResJson.val}`, headers),
          type: "hls",
          proxyDepth: 2,
          flags: [flags.CORS_ALLOWED],
          captions
        ...
        ...streamResJson.val_bak ? [
          {
            id: "backup",
            playlist: createM3U8ProxyUrl(`${baseUrl$c}/${streamResJson.val_bak}`, headers),
            type: "hls",
            flags: [flags.CORS_ALLOWED],
            proxyDepth: 2,
            captions
  ...
  const baseUrl$b = "https://wecima.tube";
  async function comboScraper$g(ctx) {
    ...
    ctx.progress(30);
    const contentPage = await ctx.proxiedFetcher(contentUrl, { baseUrl: baseUrl$b });
    ...
    if (ctx.media.type === "movie") {
      ...
      const seasonPage = await ctx.proxiedFetcher(seasonUrl, { baseUrl: baseUrl$b });
      ...
      for (const element of episodeLinks) {
        ...
        if (epTitle === `الحلقة ${ctx.media.episode}`) {
          ...
          if (episodeUrl) {
            const episodePage = await ctx.proxiedFetcher(episodeUrl, { baseUrl: baseUrl$b });
  ...
  const baseUrl$9 = "https://catflix.su";
  async function comboScraper$e(ctx) {
    ...
    const watchPageUrl = mediaType === "movie" ? `${baseUrl$9}/movie/${mediaTitle}-${movieId}` : `${baseUrl$9}/episode/${mediaTitle}-season-${ctx.media.season.number}-episode-${ctx.media.episode.number}/eid-${ctx.media.episode.tmdbId}`;
    ctx.progress(60);
  ...
  const baseUrl$7 = "https://fsharetv.co";
  async function comboScraper$c(ctx) {
    ...
    ctx.progress(50);
    const watchPage = await ctx.proxiedFetcher(watchPageUrl.replace("/movie", "/w"), { baseUrl: baseUrl$7 });
    ...
    const mediaBase = new URL((await ctx.proxiedFetcher.full(apiRes.data.file.sources[0].src, { baseUrl: baseUrl$7 })).finalUrl).origin;
  ...
  const baseUrl$5 = "https://tugaflix.best/";
